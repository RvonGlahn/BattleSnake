    ...
    def avoid_enemy(my_snake: Snake, board: BoardState, grid_map: GridMap, valid_actions: List[Direction],
        ...
        for action in valid_actions:
            ...
            if next_position.x == 0 or next_position.y == 0 or next_position.x == grid_map.width-1 \
                ...
                continue
            if next_position.x == 1 and my_head.x != 0 or next_position.y == 1 and my_head.y != 0 \
                    or next_position.x == grid_map.width-2 and my_head.x != grid_map.width-1\
                    or next_position.y == grid_map.height-2 and my_head.x != grid_map.height-1:
                cost.append(-999999999)
                continue
